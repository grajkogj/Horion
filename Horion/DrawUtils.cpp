...
void DrawUtils::drawEntityInfoTags(C_Entity* ent, float textSize, bool showPos, bool drawHealth, bool useUnicodeFont) {
	...
	std::string posText = "X:" + std::to_string((int)floorf(ent->getPos()->x)) + ", Y:" + std::to_string((int)floorf(ent->getPos()->y)) + ", Z:" + std::to_string((int)floorf(ent->getPos()->z));

	entIdMapItor = entIdMap.find(ent->getEntityTypeId());
	
	std::string idText = "Entity: " + entIdMapItor->second;
	std::string hostileText = "Hostile: ";
	if (ent->canExistInPeaceful()) {
		hostileText += "False";
	} else {
		hostileText += "True";
	}
	
	posText = Utils::sanitize(posText);
	idText = Utils::sanitize(idText);
	hostileText = Utils::sanitize(hostileText);
	

	int largestText = 0;
	std::string longestString;

	if (posText.length() > largestText) {
		largestText = posText.length();
		longestString = posText;
	} else if (idText.length() > largestText) {
		largestText = idText.length();
		longestString = idText;
	} else if (hostileText.length() > largestText) {
		largestText = hostileText.length();
		longestString = hostileText;
	};



	float textWidth = getTextWidth(&longestString, textSize);
	float textHeight = (DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() * textSize) * 3;

	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 1.5f, 0), textPos, fov, screenSize)) {
		

		static auto nameTagsMod = moduleMgr->getModule<NameTags>();

		if (ent->getEntityTypeId() != 63 && ent->getEntityTypeId() != 64) {  //make sure its not a player or item
			textPos.y -= textHeight;
			textPos.x -= textWidth / 2.f;
			vec2_t idPos = textPos;
			idPos.y += textHeight / 3;
			vec2_t hostilePos = idPos;
			hostilePos.y += textHeight / 3;
			...
			drawText(textPos, &posText, MC_Color(255, 255, 255), textSize);
			drawText(idPos, &idText, MC_Color(255, 255, 255), textSize);
			drawText(hostilePos, &hostileText, MC_Color(255, 255, 255), textSize);
